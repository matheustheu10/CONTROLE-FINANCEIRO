  ...
  Widget build(BuildContext context) {
    ...
    if (financeState.errorMessage != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(financeState.errorMessage!), backgroundColor: Colors.orange),
        );
      });
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.bar_chart_rounded), onPressed: () => Navigator.pushNamed(context, '/analysis')),
          ...
          : RefreshIndicator(
              ...
              child: CustomScrollView(
                slivers: [
                  ...
                  SliverToBoxAdapter(child: _NewsCarousel()),
                  SliverToBoxAdapter(
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          const Text('Transações', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
...
class _SkeletonLoader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          const _SkeletonBox(height: 180, radius: 24),
          const SizedBox(height: 16),
          const _SkeletonBox(height: 44, radius: 12),
          const SizedBox(height: 16),
          const _SkeletonBox(height: 120, radius: 16),
          const SizedBox(height: 16),
          ...List.generate(4, (_) => const Padding(
            padding: EdgeInsets.only(bottom: 8),
            child: _SkeletonBox(height: 76, radius: 16),
          )),
        ],
      ),
    );
    ...
    return news.when(
      loading: () => const Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Notícias Financeiras', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
            SizedBox(height: 8),
      ...
      data: (articles) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) => _startTimer(articles.length));
        return Padding(
          ...
          child: Column(
            ...
            children: [
              const Row(children: [
                Icon(Icons.newspaper_rounded, color: AppTheme.primary, size: 20),
                SizedBox(width: 8),
                Text('Notícias Financeiras', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
              ]),
                      ...
                      child: Column(
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(color: AppTheme.incomeLight, borderRadius: BorderRadius.circular(6)),
                            child: Text(a.source, style: const TextStyle(fontSize: 10, color: AppTheme.primary, fontWeight: FontWeight.w700)),
                          ),
                          const SizedBox(height: 6),
                          Text(a.title, maxLines: 3, overflow: TextOverflow.ellipsis,
                              style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Color(0xFF212121), height: 1.4)),
                        ],
                      ),
                    );
                  },
              ...
              Row(
                ...
                children: List.generate(articles.length, (i) => AnimatedContainer(
                  duration: const Duration(milliseconds: 300),
                  margin: const EdgeInsets.symmetric(horizontal: 3),
                  width: _current == i ? 20 : 6,
                  height: 6,
                  decoration: BoxDecoration(
                    color: _current == i ? AppTheme.primary : const Color(0xFFCCCCCC),
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('Saldo Atual', style: TextStyle(color: Color(0xFFA5D6A7), fontSize: 14, fontWeight: FontWeight.w500)),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(color: Colors.white.withOpacity(0.15), borderRadius: BorderRadius.circular(20)),
                child: const Text('Hoje', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w500)),
              ),
            ],
          ),
          const SizedBox(height: 8),
          ...
          Row(children: [
            Expanded(child: _MiniCard(label: 'Receitas', value: state.totalIncome, icon: Icons.arrow_upward_rounded, color: const Color(0xFF81C784))),
            const SizedBox(width: 12),
            Expanded(child: _MiniCard(label: 'Despesas', value: state.totalExpense, icon: Icons.arrow_downward_rounded, color: const Color(0xFFFF8A80))),
          ]),
        ],
      ),
    );
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 8),
        _Chip(label: '📈 Receitas', isSelected: filter == FilterType.income, selectedColor: AppTheme.income,
            onTap: () => ref.read(financeProvider(userId).notifier).setFilter(FilterType.income)),
        const SizedBox(width: 8),
        _Chip(label: '📉 Despesas', isSelected: filter == FilterType.expense, selectedColor: AppTheme.expense,
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: isSelected ? [BoxShadow(color: selectedColor.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 3))] : [],
        ),
          ...
          child: Padding(
            ...
            child: Row(children: [
              Container(
                ...
                child: Center(child: Text(tx.category.emoji, style: const TextStyle(fontSize: 22))),
              ),
              const SizedBox(width: 12),
              Expanded(child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(tx.title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
